      ...
      <div className="container-page">
          ...
          <div className="mt-3 flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-6 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex gap-4">
              ...
              <div>
                ...
                <span className="mt-1.5 inline-flex items-center rounded-full bg-amber-50 px-2.5 py-0.5 text-xs font-semibold text-amber-600">
            ...
            <div className="flex gap-2">
              <Link to={`/hostels/${currentBooking.hostel.id}`}>
        ...
        <section className="mt-10">
          <div className="flex items-center justify-between">
            <h2 className="font-semibold text-ink">Saved Hostels</h2>
            <Link to="/saved" className="text-sm font-semibold text-brand-blue hover:underline">View all</Link>
          </div>
          <div className="mt-3">
            {savedHostels.length === 0 ? (
              <EmptyState
                title="No saved hostels yet."
                description="Save hostels you're interested in and compare them later."
                action={<Link to="/hostels"><Button>Explore Hostels</Button></Link>}
              />
            ) : (
              <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                {savedHostels.slice(0, 3).map((hostel) => (
                  <HostelCard key={hostel.id} hostel={hostel} isSaved={isSaved(hostel.id)} onToggleSave={toggleSaved} />
                ))}
              </div>
            )}
          </div>
        ...
        <section className="mt-10">
          ...
          <ul className="mt-3 divide-y divide-slate-100 rounded-2xl border border-slate-200 bg-white">
            {activity.map((item) => (
              <li key={item.label} className="flex items-center justify-between px-5 py-3.5 text-sm">
                <span className="text-ink">{item.label}</span>
                <span className="text-subink">{item.time}</span>
              </li>
            ))}
          </ul>
        </section>
...
function SummaryCard({ icon: Icon, label, value, accent }: { icon: typeof House; label: string; value: string; accent?: boolean }) {
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-5">
      ...
      <p className="mt-3 text-xs text-subink">{label}</p>
