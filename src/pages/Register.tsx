...
export default function Register() {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    if (!/^\S+@\S+\.\S+$/.test(normalizedEmail)) return setMessage({ type: "error", text: "Enter a valid email address, for example you@gmail.com." });
    ...
    <div className="min-h-[calc(100vh-80px)] bg-bg px-5 py-12">
      <div className="mx-auto w-full max-w-xl rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-10">
        <div className="text-center">
          ...
          <h1 className="mt-4 text-2xl font-bold text-brand-navy sm:text-3xl">Create your account</h1>
          <p className="mt-2 text-sm text-subink">Create an account to access bookings and your accommodation dashboard.</p>
        </div>
